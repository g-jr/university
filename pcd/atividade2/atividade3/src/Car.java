...
public class Car implements Runnable{
  ...
  public void run() {
    ...
    while (!outOfSight) {
      if (cartype == REDCAR ) {
        if (ypos == initY[cartype] - 2) {
          if (xpos > bridgeXLeft && xpos < bridgeXMid) {
            try {
              System.out.println("vermelho entrando na ponte");
              mutex.acquire();
            } catch (Exception e) {
              System.out.println("erro, carro vermelho, ao entrar na ponte");
            }
          } else {
            try {
              System.out.println("vermelho saindo da ponte");
              mutex.release();
            } catch (Exception e) {
              System.out.println("erro, carro vermelho, ao sair da ponte");
            }
          }
        }
      } else  {
        if (ypos == initY[cartype] + 2) {
          if (xpos < bridgeXRight && xpos > bridgeXMid) {
            try {
              System.out.println("azul entrando na ponte");
              mutex.acquire();
            } catch (Exception e) {
              System.out.println("erro, carro azul, ao entrar na ponte");
            }
          } else {
            try {
              System.out.println("azul saindo da ponte");
              mutex.release();
            } catch (Exception e) {
              System.out.println("erro, carro azul, ao sair da ponte");
            }
          }
        }
      }
